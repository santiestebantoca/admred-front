...
const fields = ref([
  ...
  'Demora Respuesta',
  'Demora del tramitador',
])
const data = computed(() => report.person.solicitudes.asTramitador.data)
const resume = computed(() => report.person.solicitudes.asTramitador.resume)
const resumeItems = computed(() => ([
  { title: 'Asignadas', value: resume.value.asignadas },
  { title: 'Terminadas', value: resume.value.terminadas },
  { title: 'Demora promedio del tramitador', value: resume.value.demoraDH },
]))
</script>
        ...
        <bs-table>
          ...
          <template #tbody>
            <tr v-for="row in data" :key="row.id">
              ...
              <td>{{ formatTime(row.hijo_en) || '-' }}</td>
              <td>{{ row.demoraReenvioDH }}</td>
              <td>{{ formatTime(row.hijo_terminado_en) || '-' }}</td>
              <td>{{ formatTime(row.respuesta_en) || '-' }}</td>
              <td>{{ row.demoraRespuestaDH }}</td>
              <td>{{ row.demoraTramitadorDH }}</td>
            </tr>
          </template>
        </bs-table>
      </div>
